...
def write_macros(a, b, c) -> None:
    """Emit tables/nulls_counts.tex.

    The three-nulls table used to be five rows of typed literals with no generator, and the
    cross-dataset ceiling among them was attributed to a companion article. Both halves of that
    are now this paper's: the ceiling is computed here from this repository's oracle_ceiling_b.csv,
    and every cell of the table is a macro, so the table cannot drift from the results again.
    """
    out = pathlib.Path(__file__).resolve().parents[1] / "tables" / "nulls_counts.tex"
    out.parent.mkdir(parents=True, exist_ok=True)
    m = {"NullCeilN": a["n_datasets"], "NullCeilBeat": a["beat"],
         "NullCeilReal": f"{a['real']:+.4f}", "NullCeilNull": f"{a['null']:+.4f}",
         "NullCeilExcess": f"{a['excess']:+.4f}"}
    for coh, key in (("colorectal", "Col"), ("lung", "Lung")):
        m[f"NullResid{key}"] = f"{b[coh][0]:+.4f}"
        real, null, exc, _ = c[coh]
        m[f"NullPat{key}Real"] = f"{real:+.4f}"
        m[f"NullPat{key}Null"] = f"{null:+.4f}"
        m[f"NullPat{key}Excess"] = f"{exc:+.4f}"
    with open(out, "w") as fh:
        fh.write("% generated by experiments/three_nulls.py -- do not edit\n")
        for k, v in m.items():
            fh.write(f"\\newcommand{{\\{k}}}{{{v}}}\n")
    print(f"\n   wrote {out.relative_to(out.parents[1])} ({len(m)} macros)")
